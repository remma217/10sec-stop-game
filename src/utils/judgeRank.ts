export type Rank = 'SS' | 'A' | 'B' | 'C' | 'D' | 'TIME_UP';

export type JudgeResult = {
  rank: Rank;
  message: string;
  color: string;
};

export function judgeRank(measuredTime: number): JudgeResult {
  // 20秒以上の場合はタイムアップ判定を最優先で返す
  if (measuredTime >= 20.00) {
    return { rank: 'TIME_UP', message: 'タイムアップ！10秒を大幅に過ぎてしまいました（記録なし）', color: '#dc3545' };
  }

  const timeDiff = parseFloat(Math.abs(measuredTime - 10.00).toFixed(2));

  if (timeDiff === 0) return { rank: 'SS', message: 'おめでとう！ぴったり10秒！', color: '#ffcc00' };
  if (timeDiff <= 0.05) return { rank: 'A', message: '素晴らしい精度！お見事！', color: '#ff4d4d' };
  if (timeDiff <= 0.20) return { rank: 'B', message: 'かなりの好記録！あと少し！', color: '#007bff' };
  if (timeDiff <= 0.50) return { rank: 'C', message: 'まずまずの記録！', color: '#28a745' };
  return { rank: 'D', message: 'まだまだ挑戦！', color: '#6c757d' };
}
